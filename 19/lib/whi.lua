...
local warehouses_list = {'warehouse', 'sophisticatedstorage:chest'} -- "pneumaticcraft:reinforced_chest"
...
function warehouse_interface.GetFromAnyWarehouse_OLD(guess, itemName, destination, itemCount, toSlot)
    if not itemCount then itemCount = 64 end
    -- COLLECT WAREHOUSE NAMES
    local warehouses_list = net.ListMultipleMatchingDevices(warehouses_list)
